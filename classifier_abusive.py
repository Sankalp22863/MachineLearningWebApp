...
class classifier_abusive:
    ...
    def customized_preprocess(self, inp_text):
    ...
    def other_features_(self, comment):
        sentiment_analyzer = SIA()
        sentiment = sentiment_analyzer.polarity_scores(comment)

        words = self.customized_preprocess(comment) #Get text only
    ...
    def classify_comments(self, comments):
        ...
        for i, t_orig in enumerate(comments):
            ...
            try:
                ...
                try:
                    ...
                    pass
            if type(s) != str: # Intially Unicode wherever str.
                fixed_comments.append(str(s, errors="ignore"))
            else:
        ...
        X = self.transformation(comments, tf_vectorizer, idf_vector, pos_vectorizer)

        print("Running classification model...")
        predicted_class = self.predictions(X, model)
    ...
    def classify(self):
        ...
        train_preds = self.classify_comments(inp_comments)

        print ("Printing predicted values: ")
        for i,c in enumerate(inp_comments):
            print(c)
            print (self.class_to_name(train_preds[i]))
        ...
        test_preds = self.classify_comments(test_comments)
        right_count = 0
